...
declaration_list : declaration_list COMMA ID
                ...
                | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD
                {
                    ...
                    if(var_type == "VOID"){
                        ...
                        $3->node->is_global = arrID->node->is_global; 
                    }
                }
                | ID
                {
                    ...
                    if( var_type ==  "VOID"){
                        ...
                        $1->node->_name =varID->node->_name  = nameID;
                        $1->node->stackOff = varID->node->stackOff;
                        $1->node->is_global = varID->node->is_global;
                    } 
                }
                | ID LTHIRD CONST_INT RTHIRD
                {
                    ...
                    if(var_type == "VOID"){
                        ...
                        $1->node->is_global = arrID->node->is_global;                         
                    }
                }
                ;
...
statement : var_declaration
            ...
            | PRINTLN LPAREN ID RPAREN SEMICOLON
            {
                ...
                { 
                    ...
                    $3->node->_name = find->node->_name; 
                    $3->node->is_global = find->node->is_global; 
                    $3->node->stackOff = find->node->stackOff; 
                }
            }
...
variable : ID
        {
            ...
            if(id) {
                $$->node->_name = nameID; 
                $$->node->stackOff = id->node->stackOff;
                $$->node->is_global = id->node->is_global;
            }
        }
        | ID LTHIRD expression RTHIRD
        {
            ...
            if(curr){
                ...
                ////----------I guess now its ok.
                $$->node->_name = curr->node->_name; 
                $$->node->stackOff = curr->node->stackOff;
                $$->node->is_global = curr->node->is_global;
                $$->node->isArr.first = 1;
                $$->node->isArr.second = curr->node->isArr.second; 
            } 
            else{
                err_sem++; 
...
simple_expression : term
                ...
                | simple_expression ADDOP term
                {
                    ...
                    $$ = curr; 
                    $$->node = new treeNode(line_count,$1->node->getfirstLine(),"simple_expression : simple_expression ADDOP term","NULL");
                    $$->node->addChild($1->node);
                    $$->node->addChild($2->node);
                    $$->node->addChild($3->node);
                    ////--------------
                    $2->node->_name = $2->getname(); 
                }
                ;
