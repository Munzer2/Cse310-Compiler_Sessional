    ...
    void codeGeneration(int t, ostream &os)
    {
        ...
        if (makeRule() == "var_declaration : type_specifier declaration_list SEMICOLON")
        {
            ...
            for (int i = 0; i < son[1]->declarationlist.size(); i++)
            {
                ...
                {
                    if (son[1]->declarationlist[i]->isArray)
                        os << "\tSUB SP, " << to_string(2 * son[1]->declarationlist[i]->arraySize) << "\n";
                    else
                        os << "\tSUB SP, 2\n";
                }
            }
        ...
        /// done
        if (makeRule() == "expression : variable ASSIGNOP logic_expression")
        { // no more condition propagation
            son[2]->codeGeneration(t + 1, os);
            son[0]->isCond = false;
            son[2]->isCond = false;
        ...
        /// done
        if (makeRule() == "rel_expression : simple_expression RELOP simple_expression")
        { // cond propagation ends;
            son[0]->codeGeneration(t + 1, os);
            os << "\tPUSH CX\n";
            son[2]->codeGeneration(t + 1, os);
        ...
        /// done
        if (makeRule() == "simple_expression : simple_expression ADDOP term")
        { // cond propagation ends
            son[0]->codeGeneration(t + 1, os);
            os << "\tPUSH CX\n";
            son[2]->codeGeneration(t + 1, os);
        ...
        /// done
        if (makeRule() == "term : term MULOP unary_expression")
        { // cond propagation ends
            son[0]->codeGeneration(t + 1, os);
            os << "\tPUSH CX\n";
            son[2]->codeGeneration(t + 1, os);
